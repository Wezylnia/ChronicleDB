...
{
    public const int GeneratorFormatVersion = 2;
        ...
        int operationCount)
    {
        ...
        if (family == ResearchWorkloadFamily.S3DeepInheritance)
        {
            return GenerateDeepInheritance(random, operationCount);
        }
        ...
        DeterministicRandom random)
    {
        ...
        var operation = family switch
        {
            ...
            ResearchWorkloadFamily.S3DeepInheritance => throw new InvalidOperationException("S3 is generated by its topology-aware sequence generator."),
    ...
    private static List<ResearchWorkloadOperation> GenerateDeepInheritance(
        DeterministicRandom random,
        int operationCount)
    {
        var operations = new List<ResearchWorkloadOperation>(operationCount);
        ...
        void Add(ResearchWorkloadOperation operation)
        {
        ...
        // Seed a key before any branch exists so every fixed parent base can inherit it.
        Add(Data(
            step,
            ResearchWorkloadOperationKind.Put,
            history: 0,
            key: 0,
            valueSize: 32 + random.Next(1024)));

        ReadOnlySpan<int> probeDepths = [1, 2, 4, 8, 16];
        for (var depth = 1; depth <= 16 && operations.Count < operationCount; depth++)
        {
            Add(Branch(
                step,
                history: depth,
            ...
            if (probeDepths.Contains(depth) && operations.Count < operationCount)
            {
                Add(Data(
                    step,
                    ResearchWorkloadOperationKind.Read,
                    history: depth,
                    key: 0,
                    valueSize: 0));
            }
        ...
        // After the complete depth matrix exists, cycle through inherited, local,
        // tombstone, and guaranteed-negative reads. This prevents a nominally deep
        // workload from measuring only negative lookups.
        var cycleIndex = 0;
        while (operations.Count < operationCount)
        {
            var group = cycleIndex / 6;
            var phase = cycleIndex % 6;
            var depth = probeDepths[group % probeDepths.Length];
            var localKey = 1 + (group % 30);
            var valueSize = 32 + random.Next(1024);
            var operation = phase switch
            {
                0 => Data(step, ResearchWorkloadOperationKind.Read, depth, key: 0, valueSize: 0),
                1 => Data(step, ResearchWorkloadOperationKind.Put, depth, localKey, valueSize),
                2 => Data(step, ResearchWorkloadOperationKind.Read, depth, localKey, valueSize: 0),
                3 => Data(step, ResearchWorkloadOperationKind.Delete, depth, localKey, valueSize: 0),
                4 => Data(step, ResearchWorkloadOperationKind.Read, depth, localKey, valueSize: 0),
                5 => Data(step, ResearchWorkloadOperationKind.Read, depth, key: 63, valueSize: 0),
                _ => throw new InvalidOperationException("Unknown S3 cycle phase."),
            };
            Add(operation);
            cycleIndex++;
        }
