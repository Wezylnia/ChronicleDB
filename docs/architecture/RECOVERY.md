...
Opening a ChronicleDB database is a recovery operation. Application work is not exposed until storage metadata, WAL, current physical state, committed version history, and persistent snapshot metadata have all been validated and reconstructed.

## Open order

1. atomically create or validate the database metadata journal;
2. validate immutable database identity and page format;
3. require WAL/snapshot files when durable capability flags say they were already initialized;
...
12. validate snapshot boundaries against retained/current history;
13. expose the database as `Open`.
