...
From v0.8 onward, branch WAL is the transaction durability authority. A branch commit performs:
...
A v0.7 branch may have metadata commit descriptors and branch-local version pages but no `branch.wal`. On the first v0.8 open, ChronicleDB validates that complete legacy history, writes an equivalent branch WAL, fsyncs it, and only then publishes the branch-local `WalInitialized` capability flag. A crash before the flag is published leaves the partial WAL non-authoritative; the next open deletes it and repeats bootstrap from the still-authoritative v0.7 state.

Once `WalInitialized` is durable, a missing branch WAL is corruption rather than a legacy-upgrade signal.

## Checkpoint interaction

After v0.9 publishes a complete retained-history checkpoint, the branch WAL may be reset to its header. Recovery then starts from the checkpoint sequence and replays only later WAL commits. A crash before reset leaves checkpoint plus the older WAL; records at or below the checkpoint are validated but not replayed. A crash after reset uses checkpoint plus the new WAL prefix.
