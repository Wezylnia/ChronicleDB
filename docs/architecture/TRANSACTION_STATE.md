...
Commit preparation follows `Active -> Preparing -> Committing -> Committed`. Abort follows `Created|Active|Preparing|Committing -> Aborting -> Aborted`. Any other transition fails deterministically. A transaction that has reached a terminal state cannot be written again.
