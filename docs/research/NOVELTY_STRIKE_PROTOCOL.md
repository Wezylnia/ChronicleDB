...
- **Status:** `NARROW / NEEDS CORRECTNESS PROTOTYPE` — FAST'05, VMware true-clone retention, OpenZFS/ONTAP secure-purge, temporal privileged-history deletion, Lethe/WAL purge ve deletion-proof prior art broad claim'leri kapatır. True `main` physical scanner güçlüdür; ancak mevcut P6 root classifier nested branch observer'larında recursive parent fallback'i modellemediği için surviving key-specific MVCC observer-closure claim'i henüz kanıtlanmış değildir. Sonraki tek meşru engineering adımı research-only observer-exact differential oracle'dır. Ayrıntı: `A8_NOVELTY_KILL.md`.
